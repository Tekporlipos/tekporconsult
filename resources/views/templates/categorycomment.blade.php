...
if (!empty($id)) {
    ...
    $comment = categoryComent::where('user_id',$id)->where('blogType',$title)->take(1)->get();
foreach ($comment as $key => $value) {
   $content =  $value->id;
}
    
    if(empty($content)){
        $comment = new categoryComent;
...
$comment->save();
       ...
       categoryComent::where('user_id',$id)->update(['message'=>$name]);
    }
  
    $comment = categoryComent::where('user_id',$id)->take(1)->get();
?>
